...
export class homeworkEVLservice {
  ...
  async find(id: string): Promise<HomeworkEVL | null> {
    const HomeworkEVL = await this.homeworkEVL.findById(id).populate("medium").populate("subject").populate("grade").exec();
    return homeworkEVL;
  }
  ...
  async query(options = {}): Promise<HomeworkEVL[]> {
    ...
    return this.homeworkEVL.find(options).populate("medium").populate("subject").populate("grade").exec();
  }
